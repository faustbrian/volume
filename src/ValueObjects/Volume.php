...
final readonly class Volume
{
     ...
     *
     * @throws DimensionArrayCountMismatchException When array count is not exactly 3
     * @throws MissingDimensionKeysException        When required keys are missing
     * @throws NonPositiveDimensionsException       When any dimension is zero or negative
     */
    public static function fromArray(array $dimensions, Unit $unit = Unit::Centimeters): self
    {
        throw_if(count($dimensions) !== 3, DimensionArrayCountMismatchException::fromCount(count($dimensions)));
        ...
        if (array_is_list($dimensions)) {
            ...
            throw_if(!array_key_exists('length', $dimensions) || !array_key_exists('width', $dimensions) || !array_key_exists('height', $dimensions), MissingDimensionKeysException::fromProvidedKeys(array_keys($dimensions)));
