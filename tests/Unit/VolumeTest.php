...
use Cline\Volume\Exceptions\DimensionArrayCountMismatchException;
use Cline\Volume\Exceptions\InvalidQuantityException;
use Cline\Volume\Exceptions\InvalidStackingFactorException;
use Cline\Volume\Exceptions\InvalidTruckWidthException;
use Cline\Volume\Exceptions\MissingDimensionKeysException;
...
describe('volume() function', function (): void {
    ...
    describe('Sad Paths', function (): void {
        test('throws exception when dimensions array has wrong count', function (): void {
            volume([120, 80]);
        })->throws(DimensionArrayCountMismatchException::class, 'Dimensions array must contain exactly 3 values');

        test('throws exception when required dimensions are missing', function (): void {
            volume(length: 120, width: 80);
        })->throws(MissingRequiredDimensionsException::class, 'Missing required dimensions: height');

        test('throws exception when only length is missing', function (): void {
            volume(width: 80, height: 100);
        })->throws(MissingRequiredDimensionsException::class, 'Missing required dimensions: length');

        test('throws exception when only width is missing', function (): void {
            volume(length: 120, height: 100);
        })->throws(MissingRequiredDimensionsException::class, 'Missing required dimensions: width');

        test('throws exception for invalid associative array keys', function (): void {
            volume(['length' => 120, 'width' => 80, 'depth' => 100]);
        })->throws(MissingDimensionKeysException::class);
    });
});
...
describe('Volume class', function (): void {
    describe('Creation', function (): void {
        ...
        })->throws(NonPositiveDimensionsException::class, 'All dimensions must be positive numbers');
    });
